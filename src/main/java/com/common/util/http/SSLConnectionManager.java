/*
 * @(#)SSLConnectionManager.java     V1.0.0      @2014年12月24日
 *
 * Project: FAPCommon
 *
 * Modify Information:
...
/**
 * @author gmc
 * SSL连接服务类。提供SSL连接的建立，数据收发
 */
public class SSLConnectionManager {

	static Log logger = LogFactory.getLog(SSLConnectionManager.class.getName());
     ...
     * @throws Exception
     */
    public String sendData(ArrayList aData, Properties aProp) throws Exception {
        String tReturn = null;
        //1、检查发送数据
        ...
        if (iOutputStream == null) {
            throw new Exception( "!!! Server connection is not created!!!");
        }
        ...
        try {
            ...
            try {
                ...
                throw new Exception( "Send reques data error:" + e.getMessage());
            }
            ...
            try {
                ...
                //发送成功接收失败视为超时
                throw new Exception( "Receive response message error:" + ex.getMessage());
            }
        }
        catch (Exception ex) {
            //6、发送成功接收失败视为超时
    ...
    public String sendData(String aServerUrl, byte[] aOutputData, String aConnTimeOut, String aReadTimeOut, Properties aProp)
        ...
        try {
            ...
            try {
                ...
                throw new Exception( "Send request error:" + e.getMessage());
            }
            ...
            try {
                ...
                //发送成功接收失败视为超时
                throw new Exception( "Read server response message error:" + ex.getMessage());
            }
        }
        catch (Exception ex) {
            //4、发送成功接收失败视为超时
...
//		iOutputStream.write(tNewLine);
        ...
        if ((tResponseCode == null) || 
			((tResponseCode.indexOf("HTTP/1.1 100") != 0) && 
			 ...
			 (tResponseCode.indexOf("HTTP/1.0 200") != 0))){
				throw new Exception( "Server response error code=[" + tResponseCode + "]"); 
			 }
	 ...
	 */
	private void createSSLConnection(String aServerURL, int aConnTimeOut, int aReadTimeOut, KeyManager[] aClientKey) throws Exception {
		SSLSocketFactory tSSLSocketFactory = null;
		...
		try {
			...
			logger.error( e);
			throw new Exception( "Init SSL env error: - " + e.getLocalizedMessage());
		}
		...
		try {
			...
			logger.error( e);
			throw new Exception( "Connect to server error: - " + e.getLocalizedMessage());
		}
	}
